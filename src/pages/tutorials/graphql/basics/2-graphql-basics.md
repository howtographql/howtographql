...
One pattern you’ll often find is that GraphQL types have unique *IDs* that are generated by the server when the objects are created. Extending our `Person` type from the before, we could add an `id` as follows:
