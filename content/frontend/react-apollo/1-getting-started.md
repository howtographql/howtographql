...
- `src`: This directory holds the source files for your GraphQL server.
  ...
  - `generated/graphcool.graphql` contains the auto-generated **Prisma schema**. The Prisma schmea defines the CRUD operations for the types in your data model and gets automatically updated when the data model changes. You should never edit it manually!
...
There also is the Prisma database API which provides full access to the data stored in the database. This one you can open by selecting the **dev** Playground in the **database** section in the left side-menu. This API is defined by the **Prisma schema** (in `server/src/generated/graphcool.graphql`).
