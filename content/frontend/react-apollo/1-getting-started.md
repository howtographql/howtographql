...
`App.js` is a component, so move it into `components`. `App.css` and `index.css` contain styles, so move them into `styles`. You also need to change the references to these files in the `index.js` and `App.js` files accordingly:
...
```
js{4}(path=".../hackernews-react-apollo/components/App.js")
