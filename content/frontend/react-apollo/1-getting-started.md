...
1. We import all the dependencies we need to wire up the Apollo client, all from
   `@apollo/client`.
1. We create the `httpLink` that will connect our `ApolloClient` instance with
   the GraphQL API. The GraphQL server will be running on
   `http://localhost:4000`.
1. We instantiate `ApolloClient` by passing in the `httpLink` and a new instance
   of an `InMemoryCache`.
1. Finally, we render the root component of our React app. The `App` is wrapped
