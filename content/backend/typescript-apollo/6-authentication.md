...
Most of the fields on the `User` type should be clear. The field worth discussing is the one called `links`. 
...
2. The `links` field needs to implement a resolve function. Previously, you only needed to implement resolvers for fields in `Query` and `Mutation`, but `User` is also a type. The resolver for the `links` field is non-trivial, meaning GraphQL can't infer it automatically as the `User` object returned from your database does not automatically contain the `links` type. So unlike the other fields in the `User`, you need to explcitly define the `links` resolver. 

3. This is the prisma query that returns the associated `links` for a certain `User` from your database. You are using the `parent` argument which contains the all the fields of `user` that you are trying to resolve. Using the `parent.id` you can fetch the appropriate `user` record from your database and return the relevant `links` by chaining in the `links()` call. 

> **Note:** If you're having a hard time understanding the theory behind the `parent` argument and non-trivial resolvers, you should review the sections at the end of [Chapter 2](../2-a-simple-query/).
...
Now, the `makeSchema` function in `schema.ts` will pick up on the new `User` type automatically. If your server is running in `dev` mode, your `schema.graphql` should be immidiately updated with the `User` type. You can also run `npm run generate` at any time to do this. 
...
> **Note:** You might have noticed the definition of `links` in the GraphQL schema is very similar to the definition of the `links` field in the `schema.prisma` file. The prisma schema language bears a lot of resemblences to the GraphQL SDL, making it easy to reason about the properties of both schema files. 
...
Now that you have defined the `User` type and updated the `Link` type in your GraphQL schema and database models, you're all set to implemenet Authentication! 
...
2. The `signup` mutation will create a new `user` record in the database as well as generate a `token` that he/she can use later. The arguments are `email`, `password` and `name`, matching those needed to create a `user` record. 
...
3. The next step is to use your `PrismaClient` instance to store the new `User` record in the database.
...
In order to do this, we are going to use a fairly idiomatic approach using the [HTTP Authorization header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Authorization). Essentially, any API request from a client with a valid JWT token will include a header called `Authorization` which will contain said token. The server will then verify the JWT token before proceeding with the API request. In case of an invalid token, the server will return an error. 
...
2. Instead of being an object, `context` is now a _function_ which returns the `context`. `Apollo-server` is smart enough to recognize this change from object to function and will execute the function (with some arguments, like the HTTP request) to resolve the final `context` object. 
...
Whenever you're now sending a query/mutation, it will carry the JWT token.
...
To verify everything worked, you can send the following `login` mutation, which also requests the links created by the user you created just now. Verify that the `links` contains the link you just created. :
...
Moreover, if you try to create a new `link` with the `Authorization` header absent or disabled, you should get a error like this:
