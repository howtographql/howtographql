...
To test it out, we need to get a token using the `TokenAuth` Mutation and use it in our Query with the `AUTHORIZATION` HTTP header, using the `JWT` prefix. Now, we are going to the Inmsonia client:
