...
This tutorial teaches you how to build a GraphQL from scratch, so the first thing you need to do is create the directory that'll hold the files of your GraphQL server!
...
1. The `resolvers` object is the _implementation_ of the GraphQL schema. Notice how its structure  is identical to the structure of the type definition inside `typeDefs`: `Query.info`.
...
Congratulations, you just implemented and successfully tested your first GraphQL server 🎉

Now, remember when we talked about the definition of the `info: String!` field and said the exclamation mark means this field could never be `null`. Well, you're in control of what the value for that field is, right? So, what happens if you return `null` instead of the actual info string in the resolver implementation? Feel free to try that out!
...
What happens here is that the underlying `graphql-js` reference implementation ensures that the return types of your resolvers adhere to the type definitions from your GraphQL schema. Put differently, it protects you from making stupid mistakes.
...
But how does that help in defining the API for a GraphQL server? Every GraphQL schema has 3 special _root types_, these are called `Query`, `Mutation` and `Subscription`. The root types correspond to the 3 operation types offered by GraphQL. The fields on these root types are called _root field_ and define the entry-points for the API.
...
This schema has one root field called `info`. When sending queries, mutations or subscriptions to a GraphQL API, these always need to start with a root field! In this case, we only have one root field, so there's really only one possible query that's accepted by the API. Let's consider a slightly more advanced example:
...
What are the API operations that can be derived from this schema definition? Well, we know that each API operation always need to start with a root field. However, we haven't learned yet what it looks like when the _type_ of a root field is itself another [object type](http://graphql.org/learn/schema/#object-types-and-fields), as is the case here where the types of the root fields are `[User!]!`, `User` and `User!`. In the `info` example from before, the type of the root field was a `String`, which is a [scalar type](http://graphql.org/learn/schema/#scalar-types).

When the type of a root field is an object type, you can furthe expand the query (or mutation/subscription) with fields of that object type. The expanded part is called _selection set_.

Here are the operations that are accepted by a GraphQL API that implements above schema:
...
- In these example, we always query `id` and `name` of the returned `User` objects. We could potentially omit either of them - however, when querying an object type it's required that you query at least one field of it in the selection set.
- For the fields in the selection set, it doesn't matter whether the type of the root field is _required_ or a _list_. All three root fields have different [type modifiers](http://graphql.org/learn/schema/#lists-and-non-null) (i.e. different combinations of being a list and/or required) for the `User` type:
  - For the `users` field, the return type `[User!]!` means it returns a _list_ (which can not be `null`) of `User` elements. The list can also not contain elements that are `null`.
  - For the `user(id: ID!)` field, the return type `User` means the the returned value could be `null`.
