...
First we have to define exception classes.
...
Custom `ExceptionHandler` need partial function which converts an exception into unified `HandledException`. Next this handled exception is internally converted into proper JSON response.
